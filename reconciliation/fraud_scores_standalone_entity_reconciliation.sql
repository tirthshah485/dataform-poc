...
WITH source_counts AS (
  SELECT "bronze_ksa.fraud_scores" AS table_name, COUNT(*) AS row_count
  FROM `bronze_ksa.fraud_scores`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_ksa.silver_fraud_scores" AS table_name, COUNT(*) AS row_count
  FROM `silver_ksa.silver_fraud_scores`
)
SELECT
