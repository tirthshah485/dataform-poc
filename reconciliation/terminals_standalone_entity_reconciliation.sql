...
WITH source_counts AS (
  SELECT "bronze_asia_south1.terminals" AS table_name, COUNT(*) AS row_count
  FROM `bronze_asia_south1.terminals`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_asia_south1.silver_terminals" AS table_name, COUNT(*) AS row_count
  FROM `silver_asia_south1.silver_terminals`
)
SELECT
