...
WITH source_counts AS (
  SELECT "bronze_ksa.merchant_transactions" AS table_name, COUNT(*) AS row_count
  FROM `bronze_ksa.merchant_transactions`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_ksa.silver_merchant_transactions" AS table_name, COUNT(*) AS row_count
  FROM `silver_ksa.silver_merchant_transactions`
)
SELECT
