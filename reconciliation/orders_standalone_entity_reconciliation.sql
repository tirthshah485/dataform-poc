...
WITH source_counts AS (
  SELECT "bronze_uae.orders" AS table_name, COUNT(*) AS row_count
  FROM `bronze_uae.orders`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_uae.silver_orders" AS table_name, COUNT(*) AS row_count
  FROM `silver_uae.silver_orders`
)
SELECT
