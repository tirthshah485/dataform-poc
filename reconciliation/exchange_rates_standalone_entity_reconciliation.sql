...
WITH source_counts AS (
  SELECT "bronze_global.exchange_rates" AS table_name, COUNT(*) AS row_count
  FROM `bronze_global.exchange_rates`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_global.silver_exchange_rates" AS table_name, COUNT(*) AS row_count
  FROM `silver_global.silver_exchange_rates`
)
SELECT
