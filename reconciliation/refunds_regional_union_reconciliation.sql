...
WITH source_counts AS (
  SELECT "bronze_ksa.refunds" AS table_name, COUNT(*) AS row_count
  FROM `bronze_ksa.refunds` UNION ALL
  SELECT "bronze_uae.refunds" AS table_name, COUNT(*) AS row_count
  FROM `bronze_uae.refunds`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_global.silver_refunds" AS table_name, COUNT(*) AS row_count
  FROM `silver_global.silver_refunds`
)
SELECT
