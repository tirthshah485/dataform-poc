...
WITH source_counts AS (
  SELECT "bronze_egy.payment_transactions" AS table_name, COUNT(*) AS row_count
  FROM `bronze_egy.payment_transactions`
),
source_total AS (
  SELECT SUM(row_count) AS row_count FROM source_counts
),
target_counts AS (
  SELECT "silver_egy.silver_payment_transactions" AS table_name, COUNT(*) AS row_count
  FROM `silver_egy.silver_payment_transactions`
)
SELECT
